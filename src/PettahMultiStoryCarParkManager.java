...
public class PettahMultiStoryCarParkManager implements CarParkManager{
    ...
    private PettahMultiStoryCarParkManager() {
        for (int i = 0 ; i < floorCount; i++){
            FloorManager f = new FloorManager(i);
            f.getFloor().setFloorNo(i);
            Slot[] slots = new Slot[slotsForFloor];
            for (int j = 0; j < slotsForFloor; j++){
                Slot s = new Slot();
                s.setIndex(j);
                s.setOccupied(false);
                s.setAvailableBikeSpaces(bikeCountForSlot);
                if (j+1 < slotsForFloor){
                    s.setNextIndex(j+1);
                }else {
                    s.setNextIndex(-1);
                }
    ...
    private synchronized void findAvailableFloorandAddVehicle(Vehicle vehicle) {
        for (int i = 0; i < floorManagers.length; i++) {
            if(floorManagers[i].findAvailabilityByVehicle(vehicle)) {
                floorManagers[i].addVehicle(vehicle);
            }
        }
    }
    ...
    public void printLongestPark() {
        ...
        System.out.println("................................................");
        System.out.println("ID Plate : "+listOfVehicle.get(0).getNoPlate());
